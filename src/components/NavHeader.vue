            ...
            <div class="navbar-right-container" style="display: flex;">
              <div class="navbar-menu-container">
                ...
                <div class="navbar-cart-container">
                  <span class="navbar-cart-count" v-if="$store.state.nickName">{{ $store.state.cartCount }}</span>
            ...
            <div class="md-content">
              <div class="confirm-tips">
                <div class="error-wrap">
                  <span class="error error-show" v-show="errorTip" v-text="errorMsg"></span>
                </div>
                <ul>
                  <li class="regi_form_input">
                    <i class="icon IconPeople"></i>
                    <input type="text" tabindex="1" name="loginname"  class="regi_login_input regi_login_input_left" placeholder="User Name" v-model="userName" @keyup.enter="login">
                  </li>
                  ...
                  <label >
                      <input type="checkbox" name="isNoLogin" @change="isNoLogin=!isNoLogin">
                      是否一天之内免登录
                    </label>
                </ul>
              </div>
        ...
        methods:{
          login(){
            if(this.userName===''||this.userPwd===''){
              ...
              body.append('userPwd',this.userPwd);
              console.log(this.isNoLogin);
              body.append('isNoLogin',this.isNoLogin);
              axios({
                method:'post',
                ...
                if(res.data === null){
                  ...
                  this.$store.commit('updateNickName',res.data.userName)
          ...
          checkLogin(){
            //检测本地的用户cookie是否还存在，存在并且服务器也保留session,会直接登录账户
            if(this.NickName ===''){
              axios({
              methods:'post',
          ...
          getCartCount(){
            axios({
              method:'get',
              url:'http://localhost:3000/users/cartCount',
              params:{},
              responseType:'json'
            })
            .then(res=>{
              if(res.data !==''){
                this.$store.commit('updateCartCount',res.data);
              }
            })
            .catch(err=>{
              console.log(err);
