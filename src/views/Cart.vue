<template>
  <div>
              ...
              <li v-for="(item ,i) in cartList" :key="i">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{check:item.checked}" @click="editCart(item,null,true)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editCart(item,'sub')">-</a>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delComfirm(item)">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="changeSelectAll" >
    ...
    <Modal :mdShow="modalConfirm" @closeMd="closeMd">
...
<script>
    ...
    export default{
        ...
        computed:{
          totalPrice(){
            return this.cartList.reduce((prev,ele)=>{
              return ele.checked===true?prev += (ele.salePrice*ele.productNum):prev;
            },0)
          },
          isAll(){
            return  this.cartList.every((ele)=>{
              return ele.checked ===true;
            })
          },
          checkedCount(){
            return   this.cartList.reduce((prev,ele)=>{
              return ele.checked ===true? prev+=1:prev;
            },0)
        ...
        methods:{
          getCartList(){
            axios({
              method:'get',
              url:'http://localhost:3000/users/cartList',
              params:{},
              responseType:'json'
            })
            .then(result=>{
          ...
          delCart(){
            ...
            body.append('productId',this.delItem.productId);
            axios({
              method:'post',
              url:'http://localhost:3000/users/delCart',
              data:body,
              responseType:'json'
            })
            .then(res=>{
              if(res.data!==''){
                alert(res.data);
                var count = this.delItem.productNum;
                this.delItem = {};
          ...
          editCart(item,changeType,isChangeSelect){
            if(changeType==='add'){
              item.productNum++;
            }
            if(changeType==='sub'){
            ...
            var body = new URLSearchParams();
            body.append('productId',item.productId);
            body.append('productNum',item.productNum),
            body.append('checked',item.checked);
            axios({
              method:'post',
              url:'http://localhost:3000/users/editCart',
              data:body,
              responseType:'json'
            })
            .then(res=>{
              console.log('更新成功');
              this.$store.commit('updateCartCount',changeType==="add"?1:-1);
            })
            .catch(err=>{
              console.log(err);
            })
           ...
           changeSelectAll(){
            //一定要先将状态保存一下，不然在对订单列表操作过程中会影响isAll(计算属性)
            var state = !this.isAll;
            this.cartList.forEach(ele=>{
              ele.checked =state ;
            })
            var body = new URLSearchParams();
            body.append('checked',state===true?1:0);
            axios({
              method:'post',
              url:'http://localhost:3000/users/toggleAll',
              data:body,
              responseType:'json'
            })
            .then(res=>{
