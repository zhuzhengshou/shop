        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item , i) in addressFilter" :key="i" v-bind:class="{'check':checkIndex===i}" @click="changeAddress(i)">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click=" delAddressId=item.addressId; isMdShow = true">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="changeDefault(item)"><i>Set default</i></a>
                    </div>
              ...
              <div class="shipping-addr-more">
                <a class="addr-more-btn up-down-btn" href="javascript:;" :class="{open:showMore}" @click="showMore=!showMore">
                  more
                  <i class="i-up-down">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :to="'/OrderConfirm/'+addressList[checkIndex].addressId">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <modal :mdShow="isMdShow" @closeMd="closeMd">
        <p slot="message">
          您是否确认要删除此地址?
        </p>
        <div slot="btnGroup">
  ...
  export default{
    ...
    computed:{
      addressFilter(){
        if(this.showMore){
          ...
          var i = this.limit>this.addressList.length?this.addressList.length:this.limit;
          return this.addressList.slice(0,i);
        }
      }
    ...
    methods:{
      getAddressList(){
        axios({
          method:'get',
          url:'http://localhost:3000/users/addressList',
          params:{},
          responseType:'json'
        })
        .then(res=>{
          if(res.data!=''){
            ...
            this.addressList.forEach((item,i)=>{
              item.isDefault?this.checkIndex = i:'';
            })
          }
        })
        .catch(err=>{
      ...
      changeDefault(item){
        ...
        body.append('addressId',item.addressId);
        axios({
          method:'post',
          url:'http://localhost:3000/users/changeDefaultAddress',
          data:body,
          responseType:'json'
        })
        .then(res=>{
      ...
      delAddress(){
        var body = new URLSearchParams();
        body.append('addressId',this.delAddressId);
        axios({
          method:'post',
          url:'http://localhost:3000/users/delAddress',
          data:body,
          responseType:'json'
        })
        .then(res=>{
